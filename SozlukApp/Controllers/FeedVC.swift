...
class FeedVC: UIViewController ,UITableViewDelegate,UITableViewDataSource{
    ...
    @IBAction func btnLogout(_ sender: Any) {
        do {
            ...
            Common.showAlert(title: "OK", message: "Logout success.", vc: self)
    ...
    func deleteItem(item: ItemModel){
        DataService.dataService.ITEM_REF.child(item.key).removeValue{(error: Error?, ref: DatabaseReference) in
            if error != nil {
                Common.showAlert(title: "Error", message: error?.localizedDescription ?? "Fatal Error", vc: self)
            }else{
                Common.showAlert(title: "OK", message: "Deletion completed successfully", vc: self)
            }
        }
    }
